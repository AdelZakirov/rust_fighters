...
class ModelCheckpoint(object):
    ...
    def __init__(self, dirname, filename_prefix,
                 save_interval=None, score_name=None,
                 n_saved=1,
        ...
        if not (save_interval is None) ^ (score_name is None):
    ...
    def __call__(self, to_save, score=None):
        if len(to_save) == 0:
        ...
        if self._score_name is not None:
            if score is None:
                raise ValueError("`score_name` is set, `score` has to be non-None")
            priority = score
        ...
        if (len(self._saved) < self._n_saved) or (self._saved[0][0] < priority):
            ...
            fname = '{}_{}{}.pth'.format(self._fname_prefix, self._iteration, suffix)
            path = os.path.join(self._dirname, fname)

            self._save(obj=to_save, path=path)
            saved_objs.append(path)
